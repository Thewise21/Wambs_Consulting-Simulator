/* Page d'accueil de la suite : choix du simulateur.
 * Chaque carte annonce le benefice concret, pas la fonction technique. */
import { OUTILS } from '../config/tools';
...
const ICONES = {
  ...
  voiture: (
    <svg className="w-6 h-6 sm:w-7 sm:h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 18.75a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 0 1-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 0 0-3.213-9.193 2.056 2.056 0 0 0-1.58-.86H14.25M16.5 18.75h-2.625m0 0V5.625c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v8.625m11.25 4.5H8.25" />
    </svg>
  ),
  maison: (
    <svg className="w-6 h-6 sm:w-7 sm:h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  cle: (
    <svg className="w-6 h-6 sm:w-7 sm:h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 15.75V18m-7.5-6.75h.008v.008H8.25v-.008ZM12 21a9 9 0 1 1 0-18 9 9 0 0 1 0 18Zm-.75-11.25a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Zm.53 1.72 5.47 5.47" />
    </svg>
  ),
  soleil: (
    <svg className="w-6 h-6 sm:w-7 sm:h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export default function ToolHub({ t, naviguer }) {
  const s = t.hub;

  return (
    <div className="step-enter space-y-6 sm:space-y-8">
      <div className="text-center pt-2 sm:pt-6">
        <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gradient mb-3 leading-tight">
          {s.headline}
        </h2>
        <p className="text-sm sm:text-base text-wambs-muted max-w-2xl mx-auto leading-relaxed">
          {s.subline}
        </p>
      </div>
      ...
      <div className="grid gap-3 sm:gap-4 sm:grid-cols-2">
        {OUTILS.map((outil) => {
          const o = s.outils[outil.id];
          if (!o) return null;
          return (
            <button
              key={outil.id}
              type="button"
              onClick={() => naviguer(outil.route)}
              className="group text-left p-4 sm:p-5 rounded-xl border border-wambs-border bg-wambs-panel
                         transition-all cursor-pointer card-hover relative overflow-hidden
                         flex flex-col min-h-[150px]"
            >
              <span
                className="absolute top-0 left-0 right-0 h-[3px] opacity-70"
                style={{ background: `linear-gradient(90deg, ${outil.accent}, transparent)` }}
              />

              <div className="flex items-start gap-3 mb-2">
                <span className="flex-shrink-0 mt-0.5" style={{ color: outil.accent }}>
                  {ICONES[outil.icone]}
                </span>
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs sm:text-sm text-wambs-muted mt-1 leading-relaxed">{o.description}</p>
                </div>
              </div>
